...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        if (strstr(bufferMQTT, "buzzer") != NULL){
        	//Mensaje para buzzer
        	uint8_t tiempo = bufferMQTT[26]-0x30;
...
static void mqtt_app_start(void)
{
    ...
    esp_mqtt_client_config_t mqttConfig = { .broker.address.uri= "mqtt://IP:8741",.credentials.client_id="Buzzer"};
...
char bufferSalida[]="{\"tipo\":\"sensor_3\",\"valor\":\"Detectado\"}";
uint8_t detectado=0;
